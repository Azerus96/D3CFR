// D2CFR-main/pybind_wrapper.cpp (ВЕРСИЯ 7.0)
...
PYBIND11_MODULE(ofc_engine, m) {
    ...
    py::class_<InferenceRequest>(m, "InferenceRequest")
        ...
        // promise не биндится напрямую, мы будем работать с ней по-другому
        .def("set_result", [](InferenceRequest &req, std::vector<float> result) {
            req.promise.set_value(result);
        });

    // Биндинг для SharedReplayBuffer остается таким же, как в Версии 5.0
    ...
    py::class_<ofc::DeepMCCFR>(m, "DeepMCCFR")
        .def(py::init<size_t, ofc::SharedReplayBuffer*, ofc::InferenceQueue*>(), 
